...
def handle_client(conn, addr):
    ...
    try:
        ...
        print(f"Получено сообщение для проверки от {client_id}: {message}")
        mc = message.replace(" ", "")
        if mc.isdigit():
            print(f"Получено допустимое сообщение от {client_id}: {message}")
            ...
            if message == message_first_line:
                ...
                print("Сообщение не совпадает с первой строкой.")
